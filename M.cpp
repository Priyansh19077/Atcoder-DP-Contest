...
void solve() {
	...
	for (int i = 1; i <= k; i++)
		dp[0][i] = 0;
	for (int i = 0; i <= k; i++) {
		prefix[0][i] = (dp[0][i] + ((i > 0) ? prefix[0][i - 1] : 0)) % MOD;
	}
	for (int i = 1; i <= n; i++) {
		for (int j = 0; j <= k; j++) {
			int right = j;
			int left = max(j - arr[i - 1], (ll)0);
			dp[i][j] = (prefix[i - 1][right] - ((left > 0) ? prefix[i - 1][left - 1] : 0)) % MOD;
		}
		for (int j = 0; j <= k; j++) {
			prefix[i][j] = (dp[i][j] + ((j > 0) ? prefix[i][j - 1] : 0)) % MOD;
		}
	}
	cout << mod_add(dp[n][k], 0, MOD) << nline;
}
int main() {
	fastio();
#ifndef ONLINE_JUDGE
